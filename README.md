...
2018년부터 2025년까지 보상관리사 기출 확보 현황을 정리하고, 확보된 2018년·2022년 PDF를 문제풀이 형식으로 만든 정적 웹페이지입니다. 1차 민법ㆍ부동산관계법규는 타 자격시험 공통 출제유형을 보상관리사 형식으로 바꾼 예상문제를 함께 제공하고, 공유 페이지와 CBTBank 5년치 기출에서 크롤링한 관련 주제도 별도 문제은행으로 풀 수 있게 구성했습니다. 여기에 기본강의 허브와 73개 단원 페이지를 추가해 교재 목차처럼 학습할 수 있게 했습니다.
...
- `index.html`: 페이지 구조
- `lecture.html`: 기본강의 허브
- `lecture-*.html`: 머리말, 1차 과목별 기본강의, 기출ㆍ연습, 부록 단원 페이지
- `exam-map.html`: 시험지도
